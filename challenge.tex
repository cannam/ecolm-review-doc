...
\begin{sloppypar}
  ...
  transform, load) data loaders for other sources. The ECOLM schema is
  detailed and fairly effective, although it has little in common with
  the ad-hoc methods of the other datasets or with wider current
  practice.
  ...
  may be used for review, query, inferencing, and format conversion.
  ...
  This approach would concentrate on compatibility with formats and
  also perhaps software used by RISM. The motivating principle would
  be that, although our sources are not all of the ``library
  standard'' indexed by RISM, we want to facilitate linkage (now or in
  the future) for those that are, and to be prepared if in the future
  RISM should grow to cover the whole area of directly represented
  lute tablature.
